...
def calc_anomalies(paths, json_output=True):
    AUX_SIZE = 78
    HIDDEN_SIZE = 8
    ...
    if json_output:
        ...
        for path, fragment_index, fragment, loss, error_message in result:
            ...
            output = f'{path} {fragment.beg()}:{fragment.end()}\t'
            if error_message:
                output += f'MESSAGE={error_message}, NAME={get_method_name(fragment)}'
            else:
                output += f'A-INDEX={loss:.2f}, NAME={get_method_name(fragment)}'
            print(output)


if len(sys.argv) <= 2:
    print("Usage: ./prototype.py <path|pull_request_id> <json|man>")
    exit(0)
...
calc_anomalies(paths, json_output)
